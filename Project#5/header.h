...
public:
	...
	void displayL3()
	{
		...
		cout << "\t\t3rd Level - Right-Left Tree: " << root->rLink->lLink->info
			<< endl;
	}
	void displayL4()
	{
		cout << "4th Level - Left Tree: " << root->lLink->lLink->lLink->info <<
			endl;
...
private:
	...
	int nodeCount(nodeType<elemType>* p) const
	{
		if (p == nullptr)
			...
			return 1 + nodeCount(p->lLink) + nodeCount(p->rLink);//+1 because of root
	}
	...
	int spCount(nodeType<elemType>* p)
	{
		if (p == nullptr)
			return 0;
		else if (p->rLink == nullptr && p->lLink != nullptr)
			return 1 + spCount(p->lLink);
		else if (p->lLink == nullptr && p->rLink != nullptr)
			return 1 + spCount(p->rLink);
		else
			return spCount(p->lLink) + spCount(p->rLink);
	}
